...
\title{Estimaci\'on y an\'alisis de datos en partidas de juegos en comunidades \emph{online}}
...
\begin{titlepage}
	...
	\vspace{-2.5cm}
	\Huge
	\noindent\color{frenchblue}Estimaci\'on y an\'alisis de datos en partidas de juegos en comunidades \emph{online}.
	\vfill
...
%Gracias a la facilidad de los humanos en imitar y acumular conocimiento en una sociedad, logramos desarrollar habilidades complejas a trav\'es del tiempo.
Los humanos desarrollan habilidades complejas a trav\'es del tiempo.
\'Esto se debe e nuestra facilidad como especie en adquirir nuevos comportamientos mediante la imitaci\'on, permitiendo a una persona a adquirir distintas capacidades. % es auto explicativo imitar, o mas?
% Debido a la facilidad que tenemos como especie en imitar, modificar y transmitir conocimientos es lo que permite a una persona a adquirir distintas capacidades.
%Esta caracter\'istica le permite a una persona adquirir distintas capacidades.
%\todo[inline]{esta frase anterior deber\'ia describir el proceso de imitaci\'on}
...
\todo[inline]{a mejores?, a los mejores?}
...
\todo[inline]{sin mencionar que se basa en TTT?}
...
%\todo[inline]{Completar la oraci\'on anterior describiendo un poco los resultado}
...
% en general estmaos hablando de poblaciones no de individuos
% no hago el seguimiento de un individuo, lo hago poblacional
% necesitas un monton de partidas y observaciones
% si tengo a un tipo con 100 partidas y una base de datos de un millon si
% el indiviudoo es un efecto secundario, no es el objetivo
% ele ejemplo del encordado del tenista me sirve para ver para atras, no en el momento
% si manana juego en polvo de ladrillo y ya tengo 20 partidas, que zapas me conviene, de que material. ahi quizas si puede
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%% Saco todo de verosimilitud y me quedo con likelihood????
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



%\todo[inline]{Parece ser que es comun que si la diferencia de habilidad hace que haya uno de handicap, se use solamente el komi 0.5. Si hay mas, puede ni usarse komi. En estos casos siempre el blanco es el de mayor rango.}
...
\todo[inline]{(XXX Propiedades de Gaussianas, solo ponerlas si las voy a nombrar m\'as adelante, si noen apendice) }
...
\todo[inline]{poner bien apendice. y aclarar como se suman.., no demostrar todo... citar....}
...
\begin{center}
	...
	\begin{figure}[H]
		...
		\tikz{ %
			...
			\node[latent, right=of fs1, xshift=1.25cm, yshift=-1cm,fill=red!50] (s1) {$x$} ; %
...
El modelo est\'a basado en gr\'aficos de factores.
...
\begin{figure}[H]
	...
	\tikz{ % trueskill
		...
		\node[latent, below=of fr,yshift=-.1cm, fill=black!30] (r) {$r$} ; %
...
\tikz{ % Ejemplo con flechitas
	\node[factor, xshift=-2cm] (fs1) {} ;
	\node[const, right=of fs1] (nfs1) {$f_{s_1}$}; %
	\node[latent, below=of fs1,yshift=-0.5cm,fill=red!50] (s1) {$s_1$} ;
...
\label{graph:fig1vs1Bis}
	\end{subfigure}
	\caption{Pasaje de mensajes para el c\'alculo de marginales para las variables de habilidad marcadas en rojo. En \subref{graph:fig1vs1} se muestra el c\'alculo de la \textit{posterior} del jugador 2 y en \subref{graph:fig1vs1Bis} se muestra el c\'alculo de la \textit{posterior} del jugador 1. Por cuesti\'on de espacio, se omiti\'o el nodo de la variable observada $r$.}
...
\todo[inline]{Chequear que ya se dijo que el beta significa la probabilidad de ganar}
...
%XXX esta bien que la diagonal de la distribucion no sea paralela a pi=sj?)
\begin{figure}[H]
	\centering
...
\todo[inline]{no tengo la demo, chau}
...
Recordando la expresi\'on de Bayes, \'esta nos permite actualizar de forma \'optima nuestras creencia sobre ciertas hip\'otesis, dado un modelo y los datos.
...
%(xxx Pafnuty Chebyshev in 1887) no se puede citar
...
\todo[inline]{citar. Tambien se puede citar algun libro o algo asi....}
...
\todo[inline]{Esteban: ni aclaro empate ya que ni lo uso...}
...
\todo[inline]{Esteban: Ests mensajes, los dejo o los mando directo a appendice?}
...
\todo[inline]{Lo dejo o va para el apendice?}
...
%\todo[inline]{para KGS la media es la misma }
...
\todo[inline]{Esteban: leer el por que un modelo y no el otro.}
