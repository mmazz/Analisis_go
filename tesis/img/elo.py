...
plt.text(pi[maxlim]-35.5*sigma/6, pi[maxlim]-17, '$p_i=p_j\ (d_{ij}=0)$')

plt.plot(pi+4, pi, c='grey', linewidth=1,linestyle='--')
plt.text(pi[maxlim]-20.5*sigma/6, pi[maxlim]-19, '$d_{ij}=4$')
plt.plot(pi+8, pi, c='grey', linewidth=1, linestyle='--')
plt.text(pi[maxlim]-17*sigma/6, pi[maxlim]-21, '$d_{ij}=8$')

levels = np.linspace(0, 0.01, 6)
img = plt.contourf(pi, pj, Matrix, levels=levels, cmap='Blues')
plt.colorbar(img)
plt.contour(pi, pj, Matrix, levels=levels, linewidths=0.5, cmap='bone_r', vmin=z_min, vmax=z_max)
...
plt.xlabel(r'Performance $\ p_i$')
plt.ylabel(r'Performance $\ p_j$')
