...
gamma = 0.015
plt.figure(1)
plt.plot("gammas", "evidencias_ts", data=df,c='firebrick',linewidth=2, label="TrueSkill")
plt.plot("gammas", "evidencias_ttt", data=df,c='steelblue',linewidth=2, label="TTT")
plt.scatter(mints, df['evidencias_ts'].min(), data=df,c='orange',linewidth=2)
plt.scatter(minttt, df['evidencias_ttt'].min(), data=df,c='cyan',linewidth=2)
...
df = pd.read_csv(csv)
plt.figure(2)
plt.plot("mean_agent", data=df,c='firebrick',linewidth=2, label="Truth")
plt.plot("lc_0005", data=df,c='purple',linewidth=1,linestyle='--', label="$\gamma$ = 0.005")
plt.plot("lc_0015", data=df,c='steelblue',linewidth=2, label="$\gamma$ = 0.015")
plt.plot("lc_01", data=df,c='olive',linewidth=1,linestyle='--', label="$\gamma$ = 0.1")
