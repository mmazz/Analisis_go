...
numero_partidas = 500
Jugadores = Dict{String, Array{Float32,1}}("pepe" => [MU, 0], "juan" => [MU+4*BETA, 0], "ruben" => [MU+8*BETA, 0], "lol" => [MU+12*BETA, 0])
...
df = DataFrame(data)
CSV.write("./simulacion.csv", df; header=true)
...
#env=ttt.Environment(gamma=0.25 ,iter=20)
