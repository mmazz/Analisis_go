...
gammas = [gamma for gamma in 0.0001:0.0001:0.01]
evidencias_ts = Float64[]
evidencias_ttt = Float64[]
#learning_curves = Vector{Vector{Float64}}()

for gamma_i in gammas#gamma=0.015

    prior_dict = Dict{String,ttt.Rating}()
    for key in Set([(row.white) for row in eachrow(data)])
        prior_dict[string(key)] = ttt.Rating(0.0,SIGMA,BETA,gamma_i)
    end
    env=ttt.Environment(gamma=gamma_i,iter=20)
...
using Plots
Plots.GRBackend()
fig1 = plot(gammas, evidencias_ts, label="TS" )  #  ylim=(-290, -275)
fig1 = xlabel!("Gamma")
fig1 = ylabel!("Evidencia")
fig1 = plot!(gammas, evidencias_ttt, label="TTT")
savefig(fig1, "./gammas.pdf")

el_gamma = gammas[argmax(evidencias_ttt)]
df = DataFrame(gammas = gammas
              ,evidencias_ts = evidencias_ts
              ,evidencias_ttt = evidencias_ttt)
#println(df)
CSV.write("./best_gamma_evidences.csv", df; header=true)
println("Gamma optimo: ", el_gamma)

prior_dict = Dict{String,ttt.Rating}()

for key in Set([(row.white) for row in eachrow(data)])
    prior_dict[string(key)] = ttt.Rating(0.0,SIGMA,BETA,el_gamma)
end
...
env=ttt.Environment(gamma=el_gamma,iter=20)
